...
summation_path <- paste("inbox/6-summation-1-user-chunk", chunk_name, ".Rdata", sep = "")
...
stopifnot(length(imputation) %% nsnp == 0)
nhaplotype_wfake <- length(imputation) %/% nsnp
stopifnot(length(dosage) == nsnp)
imputation <- imputation - dosage
