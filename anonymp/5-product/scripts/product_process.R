...
reference_path <- paste("inbox/2-reference-5-product-chunk", chunk_name, ".bin", sep = "")

# receive ppm and positions from PPM server
fromppm <- readRDS(ppm_path)
ppm <- fromppm$ppm_shared
ppm_pos <- fromppm$ppm_shared_pos
# ppm <- readBin(ppm_path, "integer", n = chunk_size, size = 2, signed = FALSE)
# receive reference_haplotypes_e5 from GDI
reference <- readBin(reference_path, "integer", n = chunk_size, size = 1L, signed = FALSE)
remove(fromppm, ppm_path, reference_path)

## Impute

imputation <- reference[ppm_pos] * ppm
